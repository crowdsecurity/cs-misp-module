...
def remove_uuids(data):
    if isinstance(data, dict):
        return {key: remove_uuids(value) for key, value in data.items() if key not in ["uuid", "object_uuid"]}
...
class TestCrowdsecModule(unittest.TestCase):
    ...
    def test_handler_invalid_attribute(self):
        request_data = json.dumps({
            "config": {"api_key": "test_api_key"},
            "attribute": {"type": "invalid-type", "value": "1.2.3.4", "uuid": "1234"}
        })
        response = handler(request_data)
        self.assertEqual(response, {"error": "Wrong input type. Please choose on of the following: ip-dst, ip-src"})
